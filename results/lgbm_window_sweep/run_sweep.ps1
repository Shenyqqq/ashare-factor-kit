# LGBM train-window sweep: h5/h20 x (3,6|6,12|12,24) months
...
foreach ($r in $runs) {
    $i++
    $tag = "lgbm_h$($r.Horizon)_w$($r.Windows -replace ',','-')"
...
if ($failures.Count -eq 0) {
    "===== All 6 LGBM window sweep runs completed $(Get-Date -Format o) =====" | Tee-Object -FilePath $Log -Append
    exit 0
} else {
