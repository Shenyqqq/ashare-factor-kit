...
| **Walk-Forward split** | purged training + embargo（AFML Ch.7）；默认两窗共用近期 val `[idx-V, idx)`、train `[idx-V-W, idx-V)`（`VAL_WINDOW_MONTHS=6`）；`val_window=0` → train `[idx-W, idx)`、无 val（仍 purge/embargo；多窗须 `wf_selection=average`）；`hold_period_to_embargo_periods()` 自动换算；**默认月度重训** `--retrain-every 4`（周频≈月；每期重训用 `1`） |
...
- **`research/`**：IC 分析 v2（`ic_analysis_v2.py` + `ic/` 子包，driver 默认）+ v1（`ic_analysis.py`，后备）；`ic/statistics.py` 含 Newey-West/BH-FDR/rolling ICIR；`ic/decay_corr.py` 含 ICIR/t/half-life 列；`ic/io.py` JSON 元数据补全；`ic/barra.py` PIT 改造；`--plot-rolling-ic` 落盘滚动 IC 折线（`research/output/figs/`）；`pbo.py` 算 PBO+DSR（接入 `logs/analyze_results.py`）。
...
24. **因子面板缓存 industry_map 指纹**：`factors/factor_cache.py` 对 industry 只签一维标识（优先 `sw_l2` Series：shape `[N]` + index 首尾）。IC 传整表 DataFrame `[N,3]`、ML/`run.py` 传 Series `[N]` 必须同指纹；勿再对 industry 用裸 `_df_signature`（会因维数分叉假 MISS、整池重算）。旧 meta 若仍写 `[N,3]` 会一次性失配后按新口径重写。
